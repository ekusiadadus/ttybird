...
| Codex runtime status | Existing 0.154.0 daemon read-only check; synthetic Unix WebSocket active/idle/input deltas, bootstrap ordering, disconnect/reconnect, idle connection reuse, PID replacement, unavailable/unknown flags, timeout, incomplete/rejected responses and no retained preview content | Embedded/older servers, instantaneous consistency or all future protocol versions |
| Parent/child display | Scoped links, cycle handling, folds, filter and selection transitions, no empty branch affordance, retained child reveal and waiting-child expansion, stable workspace ordering, cache invalidation for snapshots/search/filters/folds | Independently executing model calls for each visible child |
...
| Ghostty export snapshot | Native UUID-selected, hidden/new-output, split and closed-terminal scenarios; Unicode through libghostty-vt, multi-format clipboard preservation, generated-file cleanup; synthetic TUI selection/open/refresh dispatch without periodic retries | Exact viewport/cursor/scroll position, atomic clipboard restore, crash/timeout cleanup guarantee, other Ghostty versions |
...
| Owned terminals | Real PTY + libghostty screen, initial/output/exit stream with idle silence, detach/reattach, bounded slow subscriber and latest-frame coalescing, explicit input/paste and Ctrl-C, Ctrl+] returning to the list, resize, private IPC, no screen persistence, x stops only the selected owned terminal while the dashboard and another owned terminal remain live, verified single/bulk stop cleanup | Every coding CLI, nested job-control programs, mouse/graphics, recovery after daemon/host failure |
