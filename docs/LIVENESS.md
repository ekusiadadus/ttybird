...
The TUI hides log-only history by default. `h` or `--history` shows it explicitly. JSON snapshots retain historical evidence with null PID/TTY; `--live-only` filters by current process association, including retained sessions on live backends. It does not mean active model calls only.
